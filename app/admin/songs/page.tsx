...
export default function AdminSongsPageWrapper() {
  return (
...
function AdminSongsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [sr, ar] = await Promise.all([fetch('/api/songs'), fetch('/api/artists')]);
  ...
  useEffect(() => {
    ...
    if (action === 'add')        { setFormMode('add-song');   setForm(defaultForm); setEditingId(null); }
    if (action === 'add-artist') { setFormMode('add-artist'); setForm(defaultForm); setEditingId(null); setActiveTab('artists'); }
  }, [searchParams]);

  const showMsg = (text: string) => { setMsg(text); setTimeout(() => setMsg(''), 4000); };

  /**
   * uploadFile — direct browser → Supabase upload via signed URL.
   * No file data ever passes through the Next.js/Vercel server,
   * so there is NO 4.5 MB body limit.
   */
  const uploadFile = async (file: File, bucket: string): Promise<string> => {
    setUploadProgress(`Preparing upload for ${file.name}…`);

    // Step 1 — ask our API for a signed upload URL (sends only filename + bucket, no file data)
    const metaRes = await fetch('/api/admin/upload', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ mode: 'signed-url', bucket, filename: file.name, contentType: file.type }),
    });

    if (!metaRes.ok) {
      const ct = metaRes.headers.get('content-type') || '';
      const err = ct.includes('application/json')
        ? (await metaRes.json()).error
        : await metaRes.text();
      throw new Error(err || `Failed to get upload URL (${metaRes.status})`);
    }

    const { signedUrl, publicUrl } = await metaRes.json();

    // Step 2 — PUT the file directly to Supabase (bypasses Vercel completely)
  ...
  const handleArtistSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.artistImageFile) imageUrl = await uploadFile(form.artistImageFile, 'images');

      const payload = {
        name: form.artistName, bio: form.artistBio,
        is_group: form.artistIsGroup, category: form.artistCategory,
        image_url: imageUrl || null,
      };

      const res = await fetch(editingId ? `/api/artists/${editingId}` : '/api/artists', {
      ...
      setFormMode('none'); setForm(defaultForm); setEditingId(null);
      fetchData();
    } catch (err) {
  ...
  const handleSongSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.songAudioFile) audioUrl = await uploadFile(form.songAudioFile, 'audio');
      if (form.songImageFile) imageUrl = await uploadFile(form.songImageFile, 'images');
      if (!audioUrl && !editingId) throw new Error('Audio file is required');

      const payload = {
        title: form.songTitle, artist_id: form.songArtistId,
        audio_url: audioUrl, image_url: imageUrl || null,
        lyrics: form.songLyrics || null, track_number: form.songTrackNumber,
        category: form.songCategory, language: form.songLanguage,
      };

      const res = await fetch(editingId ? `/api/songs/${editingId}` : '/api/songs', {
        method: editingId ? 'PUT' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify(payload),
  ...
  const handleDeleteSong = async (id: string) => {
    ...
    if (res.ok) { showMsg('Song deleted'); fetchData(); }
      ...
      const ct = res.headers.get('content-type') || '';
      const d = ct.includes('application/json') ? await res.json() : { error: await res.text() };
      showMsg(`Error: ${d.error || 'Delete failed'}`);
    }
  };
  ...
  const startEditSong = (song: Song) => {
    setForm({ ...defaultForm, songTitle: song.title, songArtistId: song.artist_id,
      songTrackNumber: song.track_number, songCategory: song.category,
      songLanguage: song.language, songLyrics: song.lyrics || '',
      songAudioUrl: song.audio_url, songImageUrl: song.image_url || '' });
    setEditingId(song.id); setFormMode('edit-song');
  };

  const startEditArtist = (artist: Artist) => {
    setForm({ ...defaultForm, artistName: artist.name, artistBio: artist.bio || '',
      artistIsGroup: artist.is_group, artistCategory: artist.category,
      artistImageUrl: artist.image_url || '' });
    setEditingId(artist.id); setFormMode('edit-artist');
  };

  const closeForm = () => { setFormMode('none'); setEditingId(null); setUploadProgress(''); };
  ...
  return (
    <div>
      ...
      {/* Action buttons */}
      <div className="flex gap-2 mb-4">
        <button onClick={() => { setFormMode('add-song'); setForm(defaultForm); setEditingId(null); }}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium"
          style={{ background: '#D4AF37', color: '#1a1a2e' }}>
          ➕ Add Song
        </button>
        <button onClick={() => { setFormMode('add-artist'); setForm(defaultForm); setEditingId(null); }}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium"
            ...
            <div className="p-4">
              ...
              {(formMode === 'add-artist' || formMode === 'edit-artist') ? (
                <form onSubmit={handleArtistSubmit} className="space-y-4">
                  <Field label="Name *" value={form.artistName} onChange={(v) => setForm(f => ({ ...f, artistName: v }))} required />
                  <Field label="Bio" value={form.artistBio} onChange={(v) => setForm(f => ({ ...f, artistBio: v }))} multiline />
                  <div>
                    <label className="block text-sm font-medium text-gray-600 mb-1.5">Category</label>
                    <select value={form.artistCategory} onChange={e => setForm(f => ({ ...f, artistCategory: e.target.value }))}
                      className="w-full px-4 py-3 rounded-xl border text-sm outline-none" style={{ borderColor: '#e8e8f0' }}>
                  ...
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input type="checkbox" checked={form.artistIsGroup}
                      onChange={e => setForm(f => ({ ...f, artistIsGroup: e.target.checked }))}
                      className="w-4 h-4 accent-yellow-400" />
                    <span className="text-gray-700">This is a group / choir</span>
                  </label>
                  <div>
                    <label className="block text-sm font-medium text-gray-600 mb-1.5">Image (any size)</label>
                  ...
                  {uploadProgress && <UploadBadge msg={uploadProgress} />}
                  <FormButtons submitting={submitting} onCancel={closeForm} />
                </form>
              ) : (
              /* ── Song form ──────────────────────────────── */
                <form onSubmit={handleSongSubmit} className="space-y-4">
                  <Field label="Title *" value={form.songTitle} onChange={v => setForm(f => ({ ...f, songTitle: v }))} required />
                  <div>
                    <label className="block text-sm font-medium text-gray-600 mb-1.5">Artist *</label>
                    <select value={form.songArtistId} onChange={e => setForm(f => ({ ...f, songArtistId: e.target.value }))}
                      required className="w-full px-4 py-3 rounded-xl border text-sm outline-none" style={{ borderColor: '#e8e8f0' }}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="number" min={1} max={999} value={form.songTrackNumber}
                        onChange={e => setForm(f => ({ ...f, songTrackNumber: Number(e.target.value) }))}
                        className="w-full px-4 py-3 rounded-xl border text-sm outline-none" style={{ borderColor: '#e8e8f0' }} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-600 mb-1.5">Category</label>
                      <select value={form.songCategory} onChange={e => setForm(f => ({ ...f, songCategory: e.target.value }))}
                        className="w-full px-4 py-3 rounded-xl border text-sm outline-none" style={{ borderColor: '#e8e8f0' }}>
                        <option value="new">New</option>
                        <option value="old">Old</option>
                        <option value="single">Single</option>
                  ...
                  <div>
                    ...
                    <select value={form.songLanguage} onChange={e => setForm(f => ({ ...f, songLanguage: e.target.value }))}
                      className="w-full px-4 py-3 rounded-xl border text-sm outline-none" style={{ borderColor: '#e8e8f0' }}>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-600 mb-1.5">
                      Audio File {!editingId && '*'}
                      <span className="ml-1 text-xs text-green-600 font-normal">— uploads directly to Supabase, no size limit</span>
                    </label>
                    <input type="file" accept=".mp3,.wav,.ogg,.m4a,.aac,.flac,audio/*"
                      onChange={e => setForm(f => ({ ...f, songAudioFile: e.target.files?.[0] || null }))}
                      required={!editingId}
                      className="w-full text-sm text-gray-600 file:mr-3 file:px-3 file:py-1.5 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-yellow-50 file:text-yellow-700" />
                    {form.songAudioFile && (
                      <p className="text-xs text-gray-400 mt-1">
                        {form.songAudioFile.name} — {(form.songAudioFile.size / 1024 / 1024).toFixed(2)} MB
                      </p>
                    )}
                  </div>
                  <div>
                  ...
                  <Field label="Lyrics" value={form.songLyrics} onChange={v => setForm(f => ({ ...f, songLyrics: v }))} multiline rows={6} />
                  {uploadProgress && <UploadBadge msg={uploadProgress} />}
                  <FormButtons submitting={submitting} onCancel={closeForm} />
                </form>
              )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {songs.length === 0 ? (
            <div className="text-center py-12 text-gray-400"><p>No songs yet. Add your first song!</p></div>
          ) : songs.map((song, i) => (
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {artists.length === 0 ? (
            <div className="text-center py-12 text-gray-400"><p>No artists yet. Add your first artist!</p></div>
          ) : artists.map((artist) => (
            <div key={artist.id} className="flex items-center gap-3 px-4 py-3 border-b last:border-0" style={{ borderColor: '#f0f0f8' }}>
              <div className="w-10 h-10 rounded-xl flex-shrink-0 overflow-hidden" style={{ background: '#f0f0f8' }}>
                {artist.image_url
                  ...
                  : <div className="w-full h-full flex items-center justify-center text-lg">{artist.is_group ? '👥' : '👤'}</div>}
              </div>
              <div className="flex-1 min-w-0">
...
function Field({ label, value, onChange, required, multiline, rows = 3 }: {
...
function UploadBadge({ msg }: { msg: string }) {
  return (
  ...
  return (
    <div className="flex gap-2 pt-2">
      <button type="button" onClick={onCancel}
        className="flex-1 py-2.5 rounded-xl text-sm border" style={{ borderColor: '#e8e8f0' }}>
        Cancel
      </button>
      <button type="submit" disabled={submitting}
        className="flex-1 py-2.5 rounded-xl text-sm font-semibold"
        style={{ background: submitting ? '#e0c070' : '#D4AF37', color: '#1a1a2e', opacity: submitting ? 0.8 : 1 }}>
